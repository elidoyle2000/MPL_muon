...
def graph_events(filename, event_indices, conditionDesc):
...
def viewOriginalDataFromCondition(path, fromFile=1, doCombineLeftAndRight=False, doPurify=False):
    ...
    indices = [index_in_file[thingy] for thingy in a[0] if sorted_file_index[thingy] == fromFile]
    
    path = "finalData2/*.npy"
    filenames = glob.glob(path)
    filenames.sort()

    graph_events(filenames[fromFile], indices, conditionDesc=desc)

viewOriginalDataFromCondition("full_muon_data.npz")